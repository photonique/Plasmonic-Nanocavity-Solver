...
function [T_TL_max]=aperiodic_solver_scaled( n, D, lambda, a, h )
   ...
   rhs=zeros(2*n+1,1); 
   rhs(n+1)=2*sqrt(-1); 
   %The following line is needed to obtain e_alpha for a range of depths h    %h=100; %to compare with k-domain calculations (single h value), uncomment this line 
   for i=1:length(h), 
